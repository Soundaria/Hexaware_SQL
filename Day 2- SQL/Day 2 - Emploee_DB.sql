...
--Give an alias to the first word as 'first_word'. Display the records sorted in ascending order based on first_word.(Q10)
	SELECT ID,STREET_ADDRESS,left(street_address,Charindex(' ',street_address)) as First_Word FROM LOCATION ;

--2 Write a query to display the names of all states belonging to the country Canada. Display the records sorted in 
--ascending order based on state name.Q2
	SELECT STATE FROM LOCATION WHERE country_id='CA' ORDER BY STATE ASC;

--3. Write a query to display the first name of the managers of Accounts department. Display the records sorted in
--ascending order based on manager name.(Q7).
	SELECT FIRST_NAME FROM employee WHERE department_id=1 AND ID IN (1,4,8,9,15,17,19) ORDER BY first_name ASC;
   
--9.Write a query to display the names of all countries belonging to region Europe. Display the records sorted in 
--ascending order based on country name.Q1
	SELECT NAME FROM Countries WHERE region_id=1 ORDER BY NAME ASC;

--10.Write a query to display the id,entire street address and the last word of the street address of all entries in
--the location table. Give an alias to the last word as 'last_word'. Display the records sorted in ascending order based on last_word.Q9
	SELECT ID,STREET_ADDRESS,right(street_address,Charindex(' ',reverse(street_address))) as Last_Word FROM LOCATION ;

--11.Write a query to display the first name and salary for all employees. Format the salary to be 10 characters long, left-padded with 
--the $ symbol. Display the records sorted in ascending order based on first name.(Q11).
	
	SELECT FIRST_NAME,concat('$$$$$',salary) FROM employee ORDER BY FIRST_NAME ASC;

--13.Write a query to display the first name and the last 5 characters of phone numbers of all employees. Display the 
--records sorted in ascending order based on first name. Give an alias to the last 5 characters of phone number as phone_number(Q8).
   SELECT FIRST_NAME, RIGHT(PHONE_NUMBER,5) AS PHONE_NUMBER FROM employee  ORDER BY first_name ASC;

--14. Write a query to display the first name of the managers of HR department. Display the records sorted in ascending 
--order based on manager name. Q6
  SELECT FIRST_NAME FROM employee WHERE department_id=2 AND ID IN (1,4,8,9,15,17,19) ORDER BY first_name ASC;

 --Display the emloyee details whose age is > 25.
 select id,first_name,last_name,email,phone_number,hire_datet,salary,date_of_birth,department_id,year(getdate())-year(date_of_birth) as age from employee;