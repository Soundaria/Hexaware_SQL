...
--Write a query to list the category and number of books in each category with alias name ‘NO_OF_BOOKS’.
	Select category,count(book_title) from LMS_BOOK_DETAILS group by CATEGORY
--Problem # 7:
--Write a query to display the number of books published by "Prentice Hall” with the alias name “NO_OF_BOOKS”.
	select count(book_title) as NO_OF_BOOKS from LMS_BOOK_DETAILS where publication ='Prentice hall';
...
--Problem # 9:
--Write a query to display the member id, member name, date of registration and expiry date of the members whose membership expiry date is 
--before APR 2013.
	select MEMBER_ID,MEMBER_NAME,DATE_REGISTER,DATE_EXPIRE from LMS_MEMBERS where DATE_EXPIRE<'2013-04-01';

--Problem # 10:
--write a query to display the member id, member name, date of registration, membership status of the members who registered before 
...
--Problem #11:
--Write a query to display the member id, member name who’s City is CHENNAI or DELHI. Hint: Display the member name in title case with alias name 
--'Name'. 
...
--Hint: display unique books. Use “BOOK_WRITTEN_BY” as alias name. 
	select book_title+'_is_writeen_by_'+author as BOOK_WRITTEN_BY from LMS_BOOK_DETAILS;
...
--with alias name “PHONENUMAVAILABLE”. Hint: Use ISNULL.
	select SUPPLIER_ID,SUPPLIER_NAME,iif(contact is null,'NO','YES') as PHONENUMAVAILABLE from LMS_SUPPLIERS_DETAILS;